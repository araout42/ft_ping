...
static void		get_host(void)
{
	...
	inet_ntop(g_ping.h_addrinfo->ai_family, &(((struct sockaddr_in*)(g_ping.h_addrinfo->ai_addr))->sin_addr), g_ping.env.host_str, 64);
	g_ping.sfd = socket(g_ping.h_addrinfo->ai_family, g_ping.h_addrinfo->ai_socktype, g_ping.h_addrinfo->ai_protocol);
	if (g_ping.sfd < 0)
		err(ERR_SOCKET);
	set_sockopt();
}
